...
const CARD_COLORS = ['bg-gray-900', 'bg-indigo-600', 'bg-emerald-600', 'bg-amber-600', 'bg-rose-600'];

// Card-stack presentation ("疊名片式") per the planning doc's decision
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-6 pb-24">
      ...
      {loading ? (
        ...
        <p className="mb-4 text-sm text-gray-400">還沒有任何帳戶,先新增一個吧</p>
      ) : (
          ...
          {accounts.map((acc, idx) => {
            ...
            return (
              <button
                ...
                className={`absolute inset-x-0 h-36 rounded-2xl p-4 text-left text-white shadow-lg transition-all ${
                  CARD_COLORS[idx % CARD_COLORS.length]
                }`}
                style={{
                  top: `${Math.abs(offset) * 10}px`,
