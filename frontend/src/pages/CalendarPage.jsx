              ...
              <div className="space-y-2">
                {selectedDayEvents.map((ev) => (
                  <div key={ev.id} className="flex items-center justify-between rounded-md border border-border p-2 text-sm">
                    ...
                    {actionMode === 'edit' && (
                      <button type="button" onClick={() => pickEvent(ev)} className="text-xs text-muted-foreground underline">編輯</button>
                    )}
                    {actionMode === 'delete' && (
                      <button type="button" onClick={() => pickEvent(ev)} className="text-xs text-red-500 underline">刪除</button>
                    )}
                  </div>
                ))}
              </div>
        ...
        <CardContent className="space-y-2 p-4">
          {pickingMode ? (
            <p className="py-2 text-center text-sm text-muted-foreground">
              請先在上方月曆點選日期,再從展開的行程列表挑選要{actionMode === 'edit' ? '編輯' : '刪除'}的項目
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button type="submit" className="flex-1">{actionMode === 'edit' ? '儲存修改' : '新增事件'}</Button>
                {actionMode === 'edit' && activeId && (
